...
// Bonus challenge: 
// See if you can respond to the query strings sent by the browser. You can enter those query strings manually on the end of the browser, and then wrote asome logic in your server to respond differently to various query strings. You will need to look at the format or URLs and how query strings are sent, and also how the http module parses the request object (where is this incoming data stored on the req object). This will involve looking at several docs. 

const http = require('http');
const routeContents = require('./moduleForSServer')
...
const server = http.createServer(function(req, res) {
  ...
  if (req.url === '/dog') {
    res.end(routeContents.dogRoute);
  }
  else if (req.url === '/cat') {
    res.end(`
